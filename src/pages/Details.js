    ...
    <div className='bg-light m-auto detailContainer p-3 mt-3'>
      ...
      <p className="text-dark fs-4 d-flex justify-content-end">
        ...
        <div className="d-flex justify-content-between">
          <button onClick={()=>navigate(-1)} className="border-0 fs-1 bg-transparent"><TiArrowBack/></button>
          {currentUser.displayName == userName && (
            <div className='d-flex align-items-center'>
              <button onClick={() => { deleteBlog(id); navigate("/")}} className="text-danger border-0 fs-3 px-1 bg-transparent"><FaTrashAlt/></button>
              <button
                onClick={() => {
                  editBlog(state);
                  navigate("/updateblog");
